...
def main_pay_dividends(schedule, car, min_reserves_ratio):
    for cap_bank in [
        x for x in schedule.agents if isinstance(x, Bank) and x.capital_ratio > car
    ]:
        if cap_bank.capital_ratio >= cap_bank.upper_bound_cratio:
            ...
            reserves_floor = (
                min_reserves_ratio
                * cap_bank.bank_deposits
                * cap_bank.buffer_reserves_ratio
            )
            ...
            if excess_capital < excess_reserves:
                ...
                cap_bank.bank_cum_dividend = (
                    cap_bank.bank_cum_dividend + cap_bank.bank_dividend
                )
