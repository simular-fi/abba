...
class Loan(mesa.Agent):
    ...
    def __init__(self, params):
        ...
        self.rate_quote = (
            ((1 + params.get("rfree")) - (self.rcvry_rate * self.pdef))
            / (1 - self.pdef)
            - 1
        ) * 1.2
    ...
    def get_all_variables(self):
        res = [
            ...
            (
                self.loan_approved
                if self.loan_approved is None
                else int(self.loan_approved)
            ),
            ...
            (
                self.loan_liquidated
                if self.loan_liquidated is None
                else int(self.loan_liquidated)
            ),
