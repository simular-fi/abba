...
class Bank(mesa.Agent):
    ...
    def __init__(self, params):
        ...
        self.upper_bound_cratio = 1.5 * params.get(
            "car"
        )  # set upper bound for capital ratio. set it to large number
        self.buffer_reserves_ratio = params.get(
            "buffer_reserves_ratio"
        )  # desired buffer or markup over
    ...
    def calculate_leverage_ratio(self):
        self.leverage_ratio = (
            self.equity / self.total_assets if self.total_assets != 0 else 0
        )
    ...
    def calculate_reserve_ratio(self):
        self.reserves_ratio = (
            self.bank_reserves / self.bank_deposits if self.bank_deposits != 0 else 0
        )
